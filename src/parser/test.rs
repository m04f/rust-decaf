...
//         span!(text, $text);
//         let mut $parser = Parser::new(tokens(text).map(|res| res.map(|res| res.unwrap())), |e| {
...
//         println!("parsing: {}", $text);
//         span!(text, $text);
//         let mut $parser = Parser::new(tokens(text).map(|res| res.map(|res| res.unwrap())), $err);
//     };
// }
//
// #[test]
...
//     assert_eq!(parser.expr().unwrap(), Expr::from((false, "false")));
// }
//
// #[test]
...
//     assert_eq!(
//         expr,
//         Expr::neg(Expr::neg(IntLiteral::from_decimal("0").into(), ""), ""),
//     );
// }
//
// #[test]
